...
<div class="row">
  <div class="col-md-4">
    <div class="form-group mb-3">
      <label for="image" class="form-label">Color</label><span class="fs-4 text-danger">*</span>
      <div class="dropdown">
        <button
    ...
    <div class="form-group mb-3">
      ...
      <select name="categoryParent_id" id="categoryParent_id" class="form-select">
        ...
        @forelse ($productCategories as $productCategory)
          <option value="{{ $productCategory->id }}" @selected(old('categoryParent_id') == $productCategory->id)>
...
<div class="d-sm-flex gap-3">
  <div id="imgPreview">
    @isset($productCategory->image)
      @if ($edit)
        <img
          src="{{ $productCategory->image && file_exists(public_path('storage/product-categories/' . $productCategory->image)) ? asset('storage/product-categories/' . $productCategory->image) : asset('img/no_disponible.jpg') }}"
  ...
  <div id="imgBannerPreview" class="mt-3 mt-sm-0">
    @isset($productCategory->banner)
      @if ($edit)
        <img
          src="{{ $productCategory->banner && file_exists(public_path('storage/product-categories/' . $productCategory->banner)) ? asset('storage/product-categories/' . $productCategory->banner) : asset('img/no_disponible.jpg') }}"
          alt="{{ $productCategory->name }}" class="img-thumbnail img_view_edit">
      @endif
    @endisset
  </div>
  ...
  <script>
    ...
    const imageBannerPreview = (event) => {
      ...
      reader.onload = (event) => {
        ...
        imageBanner.setAttribute('style', `object-fit:cover; width:200px; height:200px;`);
