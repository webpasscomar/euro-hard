          ...
          <h5 class="title-category gris">
            @if ($categoria !== 'categoria')
              ...
              <a href="{{ route('productos.categorias', $categoria) }}"
                ...
                href="{{ route('productos.list', [$categoria, $subcategoria->slug]) }}" class="link-breadcrumb"><span
              ...
              <div id="carouselProduct1" class="carousel slide">
                @unless (empty($producto->image_1))
                  <div class="carousel-indicators">
                    <button type="button" data-bs-target="#carouselProduct1" data-bs-slide-to="0" aria-current="true"
                      aria-label="Slide 0" class="active">
                    </button>
                    @for ($i = 1; $i <= 6; $i++)
                      @if (!empty($producto->{'image_' . $i}))
                        <button type="button" data-bs-target="#carouselProduct1" data-bs-slide-to="{{ $i }}"
              ...
              <div class="description-product">
                ...
                <div class="d-flex align-items-center gap-2 flex-wrap my-3">
                  @foreach ($producto->colors as $color)
                    <div class="rounded-circle" style="width:30px; height:30px; background-color: {{ $color->color }}">
                ...
                <div class="content-btn-detalle-products d-flex flex-lg-row flex-column">
                  @unless ($producto->instruction_file == null)
                    <a class="btn-rojo d-flex justify-content-center gap-1"
                      href="{{ asset('storage/products/' . $producto->instruction_file) }}"><i class="fas fa-download"></i>
                      PDF</a>
                    <a class="btn-gris"
                      href="{{ asset('storage/products/' . $producto->instruction_file) }}">Instructivo</a>
                  @endunless
